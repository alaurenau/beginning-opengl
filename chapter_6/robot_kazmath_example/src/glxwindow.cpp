...
bool SimpleGLXWindow::create(int width, int height, int bpp, bool fullscreen, const std::string& title)
{
    ...
    Display* mainDisplay = XOpenDisplay(NULL);
            ...
            /* make a blank cursor */
            Pixmap blank = XCreateBitmapFromData (mainDisplay, m_XWindow, data, 1, 1);
            Cursor cursor = XCreatePixmapCursor(mainDisplay, blank, blank, &dummy, &dummy, 0, 0);
            XFreePixmap (mainDisplay, blank);

            XDefineCursor(mainDisplay, m_XWindow, cursor);
